...
def test_scan_returns_dataframe():
    # Contract: scan always returns a DataFrame. If any setup fires it carries
    # Ticker/Market; if none fire it's empty. Either way it must not raise and
    # must be a DataFrame (column-agnostic w.r.t. the strategy's own columns).
